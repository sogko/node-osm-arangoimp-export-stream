...
//        if (this._stopped) logger('_parser endElement already stopped');
...
OSMArangoImpExport.prototype.start = function () {
    ...
    this._wstream.on('finish', (function () {
        logger('wstream finish', 'this._wstreamReady', this._wstreamReady);
    }).bind(this));
    this._wstream.on('close', (function () {
        logger('wstream closed');
    }).bind(this));
...
OSMArangoImpExport.prototype.__endWriteStream = function () {
    ...
    logger('__endWriteStream');
};

OSMArangoImpExport.prototype.__writeToStream = function (data) {
    if (this._stopped) logger('__writeToStream called after _stopped');
    if (this._stopped) return;
    ...
    var _write = function (d) {
        ...
        if (this._stopped) logger('__writeToStream writting after stopped');
        _writeNewLine();
        return this._wstream.write(JSON.stringify(d));
    }.bind(this);
    ...
    if (this._wstreamReady) {
        ...
        logger('__writeToStream queued data, count', this._wstreamPendingWrites.length);
